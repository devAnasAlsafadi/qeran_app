...
/// (direct) and Reject (opens the reason sheet) for one user. Owns a scoped
/// [MatchmakerUserActionsCubit] (param1: userId); pops `true` on approve/reject
/// success so the caller refreshes the list. A failure shows a snackbar and
...
Future<bool?> showMatchmakerReviewSheet(
  ...
  return showModalBottomSheet<bool>(
    ...
    builder: (_) => BlocProvider<MatchmakerUserActionsCubit>(
      ...
      child: const _ReviewActionSheet(),
    ),
  );
}
...
/// إلغاء (ghost). The active action shows an inline loader; all disable while
/// any action is in flight.
