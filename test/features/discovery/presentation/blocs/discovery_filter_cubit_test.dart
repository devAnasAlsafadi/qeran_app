...
void main() {
  ...
  group('buildPayload — range selections', () {
    test('height range emits RangeFrom[id] + RangeTo[id]', () async {
      await loadWith([_q(id: 5, type: FilterQuestionType.height, isRange: true)]);
    ...
    test('date (age range) emits RangeFrom/RangeTo with numeric values', () async {
      await loadWith([_q(id: 1, type: FilterQuestionType.date, isRange: true)]);
    ...
    test('radio emits QuestionFilters[id]=value', () async {
      await loadWith([
        _q(
          ...
          options: const [DiscoveryFilterOption(value: 'Single', display: 'عازب')],
        ),
      ]);
  ...
  group('_filterOutUnusable', () {
    test('drops unknown-type questions without options', () async {
      await loadWith([
        _q(id: 1, type: FilterQuestionType.select, options: const [
